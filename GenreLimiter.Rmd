...
---
# Reference: some code adapted from https://dataverse.harvard.edu/file.xhtml?persistentId=doi:10.7910/DVN/Q02JJQ/K5AWCU&version=2.0 to work with xml files from Discogs data dump 
...
# function to create unique pairs incorporating all values for lists of more than 2 values
...
discogs <- read_xml("discogs_20180901_masters.xml")
```
```{r}
# parse each master entry, and convert to character
masters <- as.character(xml_find_all(discogs, "//masters/master"))
...
```
```{r}
# For Hip Hop entries, make sure they have Correct data quality only 
...
rm(discogs, masters, hh_entries, correct_entries, hh_only)
```
```{r}
